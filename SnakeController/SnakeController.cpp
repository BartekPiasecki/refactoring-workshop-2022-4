      ...
      m_paused(false)
{
    ...
    if (w == 'W' and f == 'F' and s == 'S') {
        m_mapDimension = std::make_pair(width, height); //remove when finished
        
        m_foodPosition = std::make_pair(foodX, foodY); //will go to World object
        ...
        while (length--) {
            SnakeSegments::Segment seg; //changed
            istr >> seg.x >> seg.y;
...
bool Controller::isSegmentAtPosition(int x, int y) const
{
    /*return snakeSegments.m_segments.end() !=  std::find_if(snakeSegments.m_segments.cbegin(), snakeSegments.m_segments.cend(),
        ...
        return snake.isSegmentAtPosition(x, y);
}

bool Controller::isPositionOutsideMap(int x, int y) const //will be World method
{
    return x < 0 or y < 0 or x >= m_mapDimension.first or y >= m_mapDimension.second;
}

void Controller::sendPlaceNewFood(int x, int y) //will be World method
{
...
void Controller::sendClearOldFood() //will be World method
{
...
void Controller::handleTimeoutInd() //changed, not working 
{
